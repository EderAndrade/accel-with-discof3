...
static int16_t uiThresholdHigh = 1000;
static int16_t uiThresholdLow = -1000;
...
{   
    ...
    {   
		...
		if((ABS(uiXVal))>(ABS(uiYVal)))
		{
			if(uiXVal > uiThresholdHigh)
			{  
			  	// Red south
			  	GPIO.Write(eLedRed2, GPIO_PIN_SET);
			  	HAL_Delay(10);
			}
			else if(uiXVal < uiThresholdLow)
			{
				// Red north
				GPIO.Write(eLedRed1, GPIO_PIN_SET);
				HAL_Delay(10);
			}
			else
			{
		...
		{
			if(uiYVal < uiThresholdLow)
			{
				// Green east
				GPIO.Write(eLedGreen1, GPIO_PIN_SET);
				HAL_Delay(10);
			}
			else if(uiYVal > uiThresholdHigh)
			{
				// Green west
				GPIO.Write(eLedGreen2, GPIO_PIN_SET);
				HAL_Delay(10);
			} 
			else
			{ 
		...
		// All LEDs off!!!
		GPIO.Write(eLedBlue1	, GPIO_PIN_RESET);
		GPIO.Write(eLedRed1		, GPIO_PIN_RESET);
		GPIO.Write(eLedOrange1	, GPIO_PIN_RESET);
		GPIO.Write(eLedGreen1	, GPIO_PIN_RESET);
		GPIO.Write(eLedBlue2	, GPIO_PIN_RESET);
		GPIO.Write(eLedRed2		, GPIO_PIN_RESET);
		GPIO.Write(eLedOrange2	, GPIO_PIN_RESET);
		GPIO.Write(eLedGreen2	, GPIO_PIN_RESET);
        // OK
        return 0;
    }
    // Error
